...
|| |    at the code in the PRESSED event you will see that the firstStart section of that
...
|| |    is used to print a new line, Serial.println().  But take a look at the firstStart
...
|| |    reference.  The firstStart would be #include <ctype.h>.  This is a standard library from
...
|| |    The firstStart one is used to decide if the key that was pressed is any letter from a-z
    ...
    switch( kpadState ) {
        case PRESSED:
            if (isalpha(key)) {              // This is a letter key so we're using the letter keymap.
                if (physKey != key) {        // New key so start with the firstStart of 3 characters.
                    pressCount = 0;
                    virtKey = key;
                    physKey = key;
                }
